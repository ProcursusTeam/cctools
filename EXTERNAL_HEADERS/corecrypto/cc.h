...
cc_static_assert(CCERR_MAX <= 128, "Too many error codes.");

/*
  @abstract Wrap an internal error with a higher-level one.

  @discussion We model nested errors as a stack implemented in the
  bits of a signed integer. Errors are stored in eight-bit segments,
  i.e. they are byte-aligned. The least-significant byte is the
  highest-level error.

  We assume errors are no more than seven bits in width. This wastes a
  bit (the high bit of each byte is always zero), but it makes it easy
  to avoid undefined behavior.

  We are limited to four levels of error, which is unlikely to be an
  issue in practice. When we exceed this limit, the most deeply nested
  errors will fall out the bottom of the stack.

  @param err The new error.
  @param inner The existing stack of errors.
*/
CC_INLINE
int cc_error_wrap(int err, int inner)
{
    /* The mask ensures the shifted result is no wider than 31 bits. */
    inner = (0x7fffff & inner) << 8;
    return err | inner;
}
